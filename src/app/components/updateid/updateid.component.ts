...
@Component({
  selector: 'app-updateid',
  templateUrl: './updateid.component.html',
  styleUrls: ['./updateid.component.css']
})
export class UpdateidComponent {
    ...
    walletPost.subscribe(
      {
        next:(data)=>{ // executes when back end reposnds with success status code
          ...
          if(this.wallet.password==this.getwallet.password){
            this.router.navigate(['update',JSON.stringify(this.getwallet)]);
            this.incorrect= "";
          }
          else 
